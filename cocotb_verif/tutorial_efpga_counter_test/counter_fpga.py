...
def core_test(d):
    ...
    io['io_IO_i'][0].value = 1
    io['io_IO_i'][1].value = 0

    dut.reset = 0

    yield ClockCycles(dut.clock, 5)

    io['io_IO_i'][0].value = 0

    yield ClockCycles(dut.clock, 12)

    val = read_from_o(dut, io, 4)

    assert val == 11

    io['io_IO_i'][1].value = 1

    yield ClockCycles(dut.clock, 40)
